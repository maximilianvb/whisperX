...
def remove_compounds(segments, compound_words):
    ...
    compound_words_list = sorted(compound_words_cleaned.split(), key=len, reverse=True)
    
    # Remove shorter words that are contained within longer ones
    filtered_words = []
    for word in compound_words_list:
        if not any(word in longer_word for longer_word in filtered_words):
            filtered_words.append(word)
    
    compound_words_set = set(filtered_words)
    ...
    for segment in segments:
        ...
        for i, word in enumerate(words):
            ...
            if word in set_comp or predict([word])[0]:
                ...
                # fails if we get "fuck" and "fucker"
